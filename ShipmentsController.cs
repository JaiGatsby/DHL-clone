...
using SinExWebApp20326022.ViewModels;
...
namespace SinExWebApp20326022.Controllers
{
    public class ShipmentsController : Controller
    {
        ...
        public ActionResult GenerateHistoryReport(int? ShippingAccountId, string sortOrder, int? currentShippingAccountId, int? page, int? SDay, int? SMonth, int? SYear, int? EDay, int? EMonth, int? EYear)
        {
            ...
            if (!(SDay == null || EDay == null || SYear ==null || EYear==null || SMonth==null || EMonth==null))
            {
                var sDate = new DateTime((int)SYear, (int)SMonth, (int)SDay);
                var eDate = new DateTime((int)EYear, (int)EMonth, (int)EDay);
                if (eDate >= sDate)
                {
                    shipmentQuery = from s in shipmentQuery
                                    where (s.ShippedDate >= sDate && s.ShippedDate <= eDate)
                                    select s;
                }
